...
template <typename vertex_t, typename T, typename edge_t, typename return_type_t, bool multi_gpu,bool use_dcs>
struct local_degree_op_t_subway {
  raft::device_span<edge_t const> offsets{};
  T sub_vertex;
  std::conditional_t<multi_gpu, vertex_t, std::byte /* dummy */> major_range_first{};
 ...
 public:
                                 ...
                                 const size_t size_ = 0) const
  {
    ...
    return thrust::transform_reduce(
      ...
      majors.begin(),
      majors.end(),
      detail::local_degree_op_t_subway<vertex_t,
                                int*,
                                edge_t,
                                size_t /* no limit on majors.size(), so edge_t can overflow */,
                                multi_gpu,
