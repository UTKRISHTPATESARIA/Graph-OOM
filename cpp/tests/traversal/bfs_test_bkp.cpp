  ...
  void run_current_test(BFS_Usecase const& bfs_usecase, input_usecase_t const& input_usecase)
  {
    ...
    */
    if (bfs_usecase.check_correctness) {
      ...
      std::ofstream fout;
      fout.open("bfs_reference.txt");
      for(vertex_t i : h_reference_distances)
        fout << i << "\n";
      fout.close();
      fout.open("bfs_reference_pred.txt");
      for(vertex_t i : h_reference_predecessors)
        fout << i << "\n";
      fout.close();
       /* 
...
INSTANTIATE_TEST_SUITE_P(
  ...
  ::testing::Values(
    ...
    std::make_tuple(BFS_Usecase{1},
                    cugraph::test::File_Usecase("/home/ankit/rapids/graph/data/700m_twitter-2010.csv"))));
