 ...
 *
 * @throw cugraph::logic_error if t he `allocation_mode` is unsupported.
 *
...
inline cugraph::test::BFS_Usecase override_Source_Node_with_cmd_line_arguments(cugraph::test::BFS_Usecase usecase)
{
  if(g_source_node) {usecase.source = g_source_node;}
  return usecase;
}

inline bool get_subway_flag()
{
  return g_subway;
}
...
inline auto parse_test_options(int argc, char** argv)
{
  try {
    ...
    options.allow_unrecognised_options().add_options()(
      ...
      ("source_node", "override the hardcoded source node", cxxopts::value<size_t>()->default_value("0"))(
      "subway", "Flag to switch on/off subway", cxxopts::value<bool>()->default_value("false")
      );
...
#define CUGRAPH_TEST_PROGRAM_MAIN()                                                     \
  ...
  {                                                                                     \
    ...
    cugraph::test::g_subway =                                                           \
      (cmd_opts.count("subway") > 0)                                                    \
        ? cmd_opts["subway"].as<bool>()                         \
        : false;                                                          \
    cugraph::test::g_source_node = (cmd_opts["source_node"].as<size_t>() > 0)     \
        ? std::size_t(cmd_opts["source_node"].as<size_t>())               \
        : 0;                                                         \
  ...
  /*
  rmm::cuda_stream_view stream = rmm::cuda_stream_view{};                             \
    rmm::device_buffer device_buff1(1UL<<30, stream);                               \
    rmm::device_buffer device_buff2(1UL<<30, stream);\
    rmm::device_buffer device_buff3(1UL<<30, stream);\
    rmm::device_buffer device_buff4(1UL<<30, stream);\
    rmm::device_buffer device_buff5(1UL<<30, stream);\
    rmm::device_buffer device_buff6(1UL<<30, stream);\
    rmm::device_buffer device_buff7(1UL<<30, stream);\
    rmm::device_buffer device_buff8(1UL<<30, stream);\
    rmm::device_buffer device_buff9(1UL<<30, stream);\
    rmm::device_buffer device_buff10(1UL<<30, stream);            \
  */
